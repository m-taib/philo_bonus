...
/*   Updated: 2023/05/23 17:40:15 by mtaib            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    *philo_routine(void *philo)
{
	...
	while (1)
	{
		...
		if (arg->philo_info->info[philo_n] > 1)
		{
			...
			if (!ft_usleep(philo, arg->philo_info->info[time_to_eat]))
			{
			...
			pthread_mutex_unlock(arg->next->ptr_m);
			ft_sleep(arg);
			if (!ft_usleep(philo, arg->philo_info->info[time_to_sleep]))
			{
				pthread_mutex_unlock(arg->ptr_m);
				pthread_mutex_unlock(arg->next->ptr_m);
				return (0);
	...
	while (1)
	{
		...
		while (tmp)
		{
			...
			if ((ft_interval(philo->last_meal,
				get_time()) >= philo->philo_info->info[time_to_die]) || is_full(philo))
			{
				if (is_full(philo))
					++i;
				else
					{
						...
						printf("%ld %d died\n",
							ft_interval(philo->philo_info->init_time, get_time()), i + 1);
					}
...
void	philos_create(t_philo *philos)
{
	...
	while (philos)
	{
		...
		if (pthread_create(&philos->philo_info->ptr_t[i], NULL, philo_routine, philos))
			return ;
		philos = philos->next;
		if (philos == tmp)
			break;
